...
my %Seq = _store_seq("/storage1/data11/TYMEFLIES_phage/All_phage_species_rep_gn_containing_AMG.genes");
...
open IN, "/storage1/data11/TYMEFLIES_phage/MetaPop_idcutoff_92/VIBRANT_prophage_coordinates.txt";
while (<IN>){
	chomp;
	if (!/^scaffold/){
		my @tmp = split (/\t/);
...
open OUT, ">/storage1/data11/TYMEFLIES_phage/All_phage_species_rep_gn_containing_AMG.mdfed.genes";
