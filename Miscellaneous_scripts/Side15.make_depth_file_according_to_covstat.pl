...
# Aim: Make depth file according to "*.covstat" file in 32 metagenome folders

open IN, "find /storage1/data11/TYMEFLIES_phage/ -maxdepth 2 -type f -name '*.covstat' | ";
while (<IN>){
	chomp;
	my $file = $_;
	my ($img_id) = $file =~ /^.+\/(.+?)\.covstat/;
