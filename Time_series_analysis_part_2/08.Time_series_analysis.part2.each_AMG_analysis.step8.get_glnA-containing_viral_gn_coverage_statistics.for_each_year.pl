...
# Aim: Get the glnA-containing viral gn coverage statistics (the distribution of viral gn should be >= 1 out of 20 years) for each year
...
open OUT, ">MetaPop.for_each_year/MetaPop/GlnA_containing_viral_gn2distribution_n_cov_mean_n_tax_n_host_tax.txt";
...
# Step 4 Get glnA-containing viral gn 2 year 2 cov_ratio result 
...
my %GlnA_containing_viral_gn2year2cov = (); # $glnA_containing_viral_gn => $year => $cov
...
while (<IN>){
	...
	if (/^Head/){
		...
		my @tmp = split (/\t/);
		my $viral_gn = $tmp[0];
		if (exists $GlnA_containing_viral_gn{$viral_gn} and $GlnA_containing_viral_gn2distribution_n_cov_mean{$viral_gn}[0] >= 1){	
			for(my $i=1; $i<=$#tmp; $i++){
				my $year = $Header[$i];
				my $cov = $tmp[$i];
				$GlnA_containing_viral_gn2year2cov{$viral_gn}{$year} = $cov;
			}
		}	
	}
}
...
open OUT, ">MetaPop.for_each_year/MetaPop/GlnA_containing_viral_gn2year2cov.txt";
my $row=join("\t", @Year);
print OUT "Head\t$row\n";
foreach my $tmp1 (sort keys %GlnA_containing_viral_gn2year2cov){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Year) {       
                if (exists $GlnA_containing_viral_gn2year2cov{$tmp1}{$tmp2}){
                        push @tmp, $GlnA_containing_viral_gn2year2cov{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"0";
                }
        }
...
# Step 5 Get glnA AMG gene 2 year 2 cov ratio result
## Step 5.1 Get glnA AMG gene 2 year 2 cov ratio result
my %GlnA_AMG_gene2year2cov_ratio = (); # $glnA_amg_gene => $year => $cov_ratio
...
while (<IN>){
	...
	if (/^Head/){
		...
		# 1) $amg_gene is glnA 2) $amg_gene is in a glnA containing viral gn 3) this viral gn has distribution >= 1 out of 20 years
		if ($AMG_summary{$amg_gene} eq "K01915" and exists $GlnA_containing_viral_gn{$viral_gn} and $GlnA_containing_viral_gn2distribution_n_cov_mean{$viral_gn}[0] >= 1){	
			for(my $i=1; $i<=$#tmp; $i++){
				my $year = $Header3[$i];
				my $cov_ratio = $tmp[$i];
				if ($cov_ratio eq "Viral gn absent, AMG present" or $cov_ratio eq "Both viral gn and AMG absent"){
					$cov_ratio = 0;
					$GlnA_AMG_gene2year2cov_ratio{$amg_gene}{$year} = $cov_ratio;
				}else{
					$GlnA_AMG_gene2year2cov_ratio{$amg_gene}{$year} = $cov_ratio;
				}
			}
...
open OUT, ">MetaPop.for_each_year/MetaPop/GlnA_AMG_gene2year2cov_ratio.txt";
my $row3=join("\t", @Year);
print OUT "Head\t$row3\n";
foreach my $tmp1 (sort keys %GlnA_AMG_gene2year2cov_ratio){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Year) {       
                if (exists $GlnA_AMG_gene2year2cov_ratio{$tmp1}{$tmp2}){
                        push @tmp, $GlnA_AMG_gene2year2cov_ratio{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"0";
                }
        }
