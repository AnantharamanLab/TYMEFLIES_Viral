...
# Aim: Get the pmoC-containing viral gn coverage statistics (the distribution of viral gn should be >= 5 out of 465 metagenomes)
...
# Step 4 Get pmoC-containing viral gn 2 month 2 cov_ratio result and 2 year_month 2 cov_ratio result
...
## Step 4.1 Get pmoC-containing viral gn 2 month 2 cov result
my %PmoC_containing_viral_gn2month2cov = (); # $pmoC_containing_viral_gn => $month => $cov
	...
	if (/^Head/){
		...
		if (exists $PmoC_containing_viral_gn{$viral_gn} and $PmoC_containing_viral_gn2distribution_n_cov_mean{$viral_gn}[0] >= 5){	
			for(my $i=1; $i<=$#tmp; $i++){
				my $month = $Header[$i];
				my $cov = $tmp[$i];
				$PmoC_containing_viral_gn2month2cov{$viral_gn}{$month} = $cov;
			}
		}	
	}
}
...
open OUT, ">MetaPop/PmoC_containing_viral_gn2month2cov.txt";
my $row=join("\t", @Month);
print OUT "Head\t$row\n";
foreach my $tmp1 (sort keys %PmoC_containing_viral_gn2month2cov){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Month) {       
                if (exists $PmoC_containing_viral_gn2month2cov{$tmp1}{$tmp2}){
                        push @tmp, $PmoC_containing_viral_gn2month2cov{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"0";
                }
        }
...
## Step 4.2 Get pmoC-containing viral gn 2 year_month 2 cov result
my %PmoC_containing_viral_gn2year_month2cov = (); # $pmoC_containing_viral_gn => $year_month => $cov
	...
	if (/^Head/){
		...
		if (exists $PmoC_containing_viral_gn{$viral_gn} and $PmoC_containing_viral_gn2distribution_n_cov_mean{$viral_gn}[0] >= 5){	
			for(my $i=1; $i<=$#tmp; $i++){
				...
				my $cov = $tmp[$i];
				$PmoC_containing_viral_gn2year_month2cov{$viral_gn}{$year_month} = $cov;
			}
		}	
	}
}
...
open OUT, ">MetaPop/PmoC_containing_viral_gn2year_month2cov.txt";
my $row2=join("\t", @Year_month);
print OUT "Head\t$row2\n";
foreach my $tmp1 (sort keys %PmoC_containing_viral_gn2year_month2cov){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Year_month) {       
                if (exists $PmoC_containing_viral_gn2year_month2cov{$tmp1}{$tmp2}){
                        push @tmp, $PmoC_containing_viral_gn2year_month2cov{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"0";
                }
        }
...
# Step 5 Get pmoC AMG gene 2 month 2 cov result and 2 year_month 2 cov ratio result
## Step 5.1 Get pmoC AMG gene 2 month 2 cov ratio result
my %PmoC_AMG_gene2month2cov_ratio = (); # $pmoC_amg_gene => $month => $cov_ratio
...
while (<IN>){
	...
	if (/^Head/){
		...
		# 1) $amg_gene is pmoC 2) $amg_gene is in a pmoC containing viral gn 3) this viral gn has distribution >= 5 out of 465 metagenomes
		if ($AMG_summary{$amg_gene} eq "K10946" and exists $PmoC_containing_viral_gn{$viral_gn} and $PmoC_containing_viral_gn2distribution_n_cov_mean{$viral_gn}[0] >= 5){	
			for(my $i=1; $i<=$#tmp; $i++){
				my $month = $Header3[$i];
				my $cov_ratio = $tmp[$i];
				$PmoC_AMG_gene2month2cov_ratio{$amg_gene}{$month} = $cov_ratio;
			}
		}	
	}
}
...
open OUT, ">MetaPop/PmoC_AMG_gene2month2cov_ratio.txt";
my $row3=join("\t", @Month);
print OUT "Head\t$row3\n";
foreach my $tmp1 (sort keys %PmoC_AMG_gene2month2cov_ratio){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Month) {       
                if (exists $PmoC_AMG_gene2month2cov_ratio{$tmp1}{$tmp2}){
                        push @tmp, $PmoC_AMG_gene2month2cov_ratio{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"0";
                }
        }
...
## Step 5.2 Get pmoC AMG gene 2 year_month 2 cov ratio result
my %PmoC_AMG_gene2year_month2cov_ratio = (); # $pmoC_amg_gene => $year_month => $cov_ratio
	...
	if (/^Head/){
		...
		if ($AMG_summary{$amg_gene} eq "K10946" and exists $PmoC_containing_viral_gn{$viral_gn} and $PmoC_containing_viral_gn2distribution_n_cov_mean{$viral_gn}[0] >= 5){	
			for(my $i=1; $i<=$#tmp; $i++){
				...
				my $cov_ratio = $tmp[$i];
				$PmoC_AMG_gene2year_month2cov_ratio{$amg_gene}{$year_month} = $cov_ratio;
			}
		}	
	}
}
...
open OUT, ">MetaPop/PmoC_AMG_gene2year_month2cov_ratio.txt";
my $row4=join("\t", @Year_month);
print OUT "Head\t$row4\n";
foreach my $tmp1 (sort keys %PmoC_AMG_gene2year_month2cov_ratio){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Year_month) {       
                if (exists $PmoC_AMG_gene2year_month2cov_ratio{$tmp1}{$tmp2}){
                        push @tmp, $PmoC_AMG_gene2year_month2cov_ratio{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"0";
                }
        }
