...
# Step 1 Store interpolation results for the first figure of UBA6136_MAG
## Store the highest y1 value mean
highest_y1_values = []
with open(f"{Path(output_directory) / 'UBA6136_MAG.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
interpolation_result_files1 = glob(f"{Path(output_directory) / '*.UBA6136_MAG.interpolation_results.txt'}")
x_data1 = x_mean
...
# Step 2 Store interpolation results for the second figure of pmoC_containing_UBA6136_viral_gn
## Store the highest y2 value mean
highest_y2_values = []
with open(f"{Path(output_directory) / 'pmoC_containing_UBA6136_viral_gn.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
## Store interpolation results
interpolation_result_files2 = glob(f"{Path(output_directory) / '*.pmoC_containing_UBA6136_viral_gn.interpolation_results.txt'}")
x_data2 = x_mean
...
# Step 3 Store interpolation results for the third figure of no_pmoC_containing_UBA6136_viral_gn
## Store the highest y3 value mean
highest_y3_values = []
with open(f"{Path(output_directory) / 'no_pmoC_containing_UBA6136_viral_gn.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
## Store interpolation results
interpolation_result_files3 = glob(f"{Path(output_directory) / '*.no_pmoC_containing_UBA6136_viral_gn.interpolation_results.txt'}")
x_data3 = x_mean
...
## Plot the second and third mean line (pmoC_containing_UBA6136_viral_gn + no_pmoC_containing_UBA6136_viral_gn)
...
ax2 = plt.gca().twinx()
ax2.set_ylim(0, 150)  # Set the y-axis limits for the third mean line
...
plt.savefig('UBA6136_viral_gn.combined.mean_lines.pdf')


# Step 5 Plotting the first mean line together in one figure
pltt.figure(figsize=(5, 2.5))
pltt.xlim(-65, 145)
pltt.ylim(0, 20)  # Adjust the ylim to accommodate the first line
...
## Save the combined figure
pltt.savefig('UBA6136_MAG.mean_line.v2.pdf')