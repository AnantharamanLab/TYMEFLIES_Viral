...
# Step 1 Store the psbA-containing Microcystis viral gn cov files for each year
psbA_containing_Microcystis_viral_gn_cov_files = glob("/storage1/data11/TYMEFLIES_phage/virus_n_MAG_tax_association/Microcystis_virus_n_MAG/*.psbA_containing_Microcystis_viral_gn_cov.txt")
year2psbA_containing_Microcystis_viral_gn_cov = defaultdict(dict)
for psbA_containing_Microcystis_viral_gn_cov_file in psbA_containing_Microcystis_viral_gn_cov_files:
    year = Path(psbA_containing_Microcystis_viral_gn_cov_file).stem.split('.')[0]    
    x_list = []
    y_list = []
    with open(psbA_containing_Microcystis_viral_gn_cov_file, 'r') as lines:
        for line in lines:
            line = line.rstrip('\n')
    ...
    year2psbA_containing_Microcystis_viral_gn_cov[year]['x'] = x_list 
    year2psbA_containing_Microcystis_viral_gn_cov[year]['y'] = y_list       


# Step 2 Generate the mean line
## Create interpolation functions for each year's dataset
interpolation_functions = {}
for year, data in year2psbA_containing_Microcystis_viral_gn_cov.items():
...
for year, data in interpolation_results.items():
    output_filename = f"{year}.psbA_containing_Microcystis_viral_gn.interpolation_results.txt"
...
f = open('Microcystis_virus_n_MAG/psbA_containing_Microcystis_viral_gn.year2highest_y_value.txt', 'w')
...
interpolation_result_files = glob(f"{Path(output_directory) / '*.psbA_containing_Microcystis_viral_gn.interpolation_results.txt'}")
x_data = x_mean
...
plt.legend()
plt.grid(True)          
plt.savefig('psbA_containing_Microcystis_viral_gn.mean_line.pdf')
...
### Save the plot
plt.savefig('Microcystis_virus_n_MAG.latesummer_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Microcystis_virus_n_MAG.fall_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Microcystis_virus_n_MAG.clearwater_distance_barplot.pdf')
