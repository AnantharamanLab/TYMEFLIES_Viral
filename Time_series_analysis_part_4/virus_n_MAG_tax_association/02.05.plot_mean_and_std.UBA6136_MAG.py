...
# Step 1 Store the UBA6136 MAG cov files for each year
UBA6136_MAG_cov_files = glob("/storage1/data11/TYMEFLIES_phage/virus_n_MAG_tax_association/UBA6136_virus_n_MAG/*.UBA6136_MAG_cov.txt")
year2UBA6136_MAG_cov = defaultdict(dict)
for UBA6136_MAG_cov_file in UBA6136_MAG_cov_files:
    year = Path(UBA6136_MAG_cov_file).stem.split('.')[0]    
    x_list = []
    y_list = []
    with open(UBA6136_MAG_cov_file, 'r') as lines:
        for line in lines:
            line = line.rstrip('\n')
    ...
    year2UBA6136_MAG_cov[year]['x'] = x_list 
    year2UBA6136_MAG_cov[year]['y'] = y_list       


# Step 2 Generate the mean line
## Create interpolation functions for each year's dataset
interpolation_functions = {}
for year, data in year2UBA6136_MAG_cov.items():
...
for year, data in interpolation_results.items():
    output_filename = f"{year}.UBA6136_MAG.interpolation_results.txt"
...
f = open('UBA6136_virus_n_MAG/UBA6136_MAG.year2highest_y_value.txt', 'w')
...
interpolation_result_files = glob(f"{Path(output_directory) / '*.UBA6136_MAG.interpolation_results.txt'}")
x_data = x_mean
...
plt.legend()
plt.grid(True)          
plt.savefig('UBA6136_MAG.mean_line.pdf')