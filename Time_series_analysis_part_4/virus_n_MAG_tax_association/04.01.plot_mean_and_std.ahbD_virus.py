...
# Step 1 Store the ahbD virus cov files for each year
ahbD_virus_cov_files = glob("/storage1/data11/TYMEFLIES_phage/virus_n_MAG_tax_association/ahbD_virus_n_MAG/*.ahbD_viral_gn_cov.txt")
year2ahbD_viral_gn_cov = defaultdict(dict)
for ahbD_virus_cov_file in ahbD_virus_cov_files:
    year = Path(ahbD_virus_cov_file).stem.split('.')[0]    
    x_list = []
    y_list = []
    with open(ahbD_virus_cov_file, 'r') as lines:
        for line in lines:
            line = line.rstrip('\n')
    ...
    year2ahbD_viral_gn_cov[year]['x'] = x_list 
    year2ahbD_viral_gn_cov[year]['y'] = y_list       


# Step 2 Generate the mean line
## Create interpolation functions for each year's dataset
interpolation_functions = {}
for year, data in year2ahbD_viral_gn_cov.items():
...
output_directory = "ahbD_virus_n_MAG"  # Replace with your desired output directory

year2highest_y_value = {} # year => highest_y_value
for year, data in interpolation_results.items():
    output_filename = f"{year}_interpolation_results.txt"
...
f = open('ahbD_virus_n_MAG/ahbD_virus.year2highest_y_value.txt', 'w')
...
interpolation_result_files = glob(f"{Path(output_directory) / '*_interpolation_results.txt'}")
x_data = x_mean
...
plt.legend()
plt.grid(True)          
plt.savefig('ahbD_viral_gn_mean_line.pdf')
...
### Save the plot
plt.savefig('ahbD_earlysummer_distance_barplot.pdf')
...
### Save the plot
plt.savefig('ahbD_latesummer_distance_barplot.pdf')
...
### Save the plot
plt.savefig('ahbD_fall_distance_barplot.pdf')
...
### Save the plot
plt.savefig('ahbD_iceon_distance_barplot.pdf')
