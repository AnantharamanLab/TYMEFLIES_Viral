...
# Step 1 Store the pmoC-containing Methylocystis viral gn cov files for each year
pmoC_containing_Methylocystis_viral_gn_cov_files = glob("/storage1/data11/TYMEFLIES_phage/virus_n_MAG_tax_association/Methylocystis_virus_n_MAG/*.pmoC_containing_Methylocystis_viral_gn_cov.txt")
year2pmoC_containing_Methylocystis_viral_gn_cov = defaultdict(dict)
for pmoC_containing_Methylocystis_viral_gn_cov_file in pmoC_containing_Methylocystis_viral_gn_cov_files:
    year = Path(pmoC_containing_Methylocystis_viral_gn_cov_file).stem.split('.')[0]    
    x_list = []
    y_list = []
    with open(pmoC_containing_Methylocystis_viral_gn_cov_file, 'r') as lines:
        for line in lines:
            line = line.rstrip('\n')
    ...
    year2pmoC_containing_Methylocystis_viral_gn_cov[year]['x'] = x_list 
    year2pmoC_containing_Methylocystis_viral_gn_cov[year]['y'] = y_list       


# Step 2 Generate the mean line
## Create interpolation functions for each year's dataset
interpolation_functions = {}
for year, data in year2pmoC_containing_Methylocystis_viral_gn_cov.items():
...
output_directory = "Methylocystis_virus_n_MAG"  # Replace with your desired output directory

year2highest_y_value = {} # year => highest_y_value
for year, data in interpolation_results.items():
    output_filename = f"{year}.pmoC_containing_Methylocystis_viral_gn.interpolation_results.txt"
...
f = open('Methylocystis_virus_n_MAG/pmoC_containing_Methylocystis_viral_gn.year2highest_y_value.txt', 'w')
...
interpolation_result_files = glob(f"{Path(output_directory) / '*.pmoC_containing_Methylocystis_viral_gn.interpolation_results.txt'}")
x_data = x_mean
...
plt.legend()
plt.grid(True)          
plt.savefig('pmoC_containing_Methylocystis_viral_gn.mean_line.pdf')
...
### Save the plot
plt.savefig('Methylocystis_virus_n_MAG.iceon_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Methylocystis_virus_n_MAG.earlysummer_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Methylocystis_virus_n_MAG.fall_distance_barplot.pdf')
