...
# Step 1 Store interpolation results for the first figure of Other_Cyanobacteria_MAG
## Store the highest y1 value mean
highest_y1_values = []
with open(f"{Path(output_directory) / 'Other_Cyanobacteria_MAG.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
interpolation_result_files1 = glob(f"{Path(output_directory) / '*.Other_Cyanobacteria_MAG.interpolation_results.txt'}")
x_data1 = x_mean
...
# Step 2 Store interpolation results for the second figure of psbA_containing_Other_Cyanobacteria_viral_gn
## Store the highest y2 value mean
highest_y2_values = []
with open(f"{Path(output_directory) / 'psbA_containing_Other_Cyanobacteria_viral_gn.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
## Store interpolation results
interpolation_result_files2 = glob(f"{Path(output_directory) / '*.psbA_containing_Other_Cyanobacteria_viral_gn.interpolation_results.txt'}")
x_data2 = x_mean
...
# Step 3 Store interpolation results for the third figure of no_psbA_containing_Other_Cyanobacteria_viral_gn
## Store the highest y3 value mean
highest_y3_values = []
with open(f"{Path(output_directory) / 'no_psbA_containing_Other_Cyanobacteria_viral_gn.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
## Store interpolation results
interpolation_result_files3 = glob(f"{Path(output_directory) / '*.no_psbA_containing_Other_Cyanobacteria_viral_gn.interpolation_results.txt'}")
x_data3 = x_mean
...
plt.xlim(-45, 180)
plt.ylim(0, 20)  # Adjust the ylim to accommodate the first line

## Plot the second and third mean line (psbA_containing_Other_Cyanobacteria_viral_gn + no_psbA_containing_Other_Cyanobacteria_viral_gn)
...
ax2 = plt.gca().twinx()
ax2.set_ylim(0, 6000)  # Set the y-axis limits for the third mean line
...
plt.savefig('Other_Cyanobacteria_viral_gn.combined.mean_lines.pdf')
...
## Save the combined figure
pltt.savefig('Other_Cyanobacteria_MAG.mean_line.v2.pdf')