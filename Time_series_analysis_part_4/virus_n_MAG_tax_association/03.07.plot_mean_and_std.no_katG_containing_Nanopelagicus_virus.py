...
# Step 1 Store the no_katG_containing_Nanopelagicus_viral_gn_cov files for each year
no_katG_containing_Nanopelagicus_viral_gn_cov_files = glob("/storage1/data11/TYMEFLIES_phage/virus_n_MAG_tax_association/Nanopelagicus_virus_n_MAG/*.no_katG_containing_Nanopelagicus_viral_gn_cov.txt")
year2no_katG_containing_Nanopelagicus_viral_gn_cov = defaultdict(dict)
for no_katG_containing_Nanopelagicus_viral_gn_cov_file in no_katG_containing_Nanopelagicus_viral_gn_cov_files:
    year = Path(no_katG_containing_Nanopelagicus_viral_gn_cov_file).stem.split('.')[0]    
    x_list = []
    y_list = []
    with open(no_katG_containing_Nanopelagicus_viral_gn_cov_file, 'r') as lines:
        for line in lines:
            line = line.rstrip('\n')
    ...
    year2no_katG_containing_Nanopelagicus_viral_gn_cov[year]['x'] = x_list 
    year2no_katG_containing_Nanopelagicus_viral_gn_cov[year]['y'] = y_list       


# Step 2 Generate the mean line
## Create interpolation functions for each year's dataset
interpolation_functions = {}
for year, data in year2no_katG_containing_Nanopelagicus_viral_gn_cov.items():
...
for year, data in interpolation_results.items():
    output_filename = f"{year}.no_katG_containing_Nanopelagicus_viral_gn.interpolation_results.txt"
...
f = open('Nanopelagicus_virus_n_MAG/no_katG_containing_Nanopelagicus_viral_gn.year2highest_y_value.txt', 'w')
...
interpolation_result_files = glob(f"{Path(output_directory) / '*.no_katG_containing_Nanopelagicus_viral_gn.interpolation_results.txt'}")
x_data = x_mean
...
plt.legend()
plt.grid(True)          
plt.savefig('no_katG_containing_Nanopelagicus_viral_gn.mean_line.pdf')