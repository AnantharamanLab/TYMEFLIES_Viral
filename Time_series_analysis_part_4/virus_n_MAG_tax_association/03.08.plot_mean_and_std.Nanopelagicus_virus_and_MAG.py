...
# Step 1 Store interpolation results for the first figure of Nanopelagicus_MAG
## Store the highest y1 value mean
highest_y1_values = []
with open(f"{Path(output_directory) / 'Nanopelagicus_MAG.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
interpolation_result_files1 = glob(f"{Path(output_directory) / '*.Nanopelagicus_MAG.interpolation_results.txt'}")
x_data1 = x_mean
...
# Step 2 Store interpolation results for the second figure of katG_containing_Nanopelagicus_viral_gn
## Store the highest y2 value mean
highest_y2_values = []
with open(f"{Path(output_directory) / 'katG_containing_Nanopelagicus_viral_gn.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
## Store interpolation results
interpolation_result_files2 = glob(f"{Path(output_directory) / '*.katG_containing_Nanopelagicus_viral_gn.interpolation_results.txt'}")
x_data2 = x_mean
...
# Step 3 Store interpolation results for the third figure of no_katG_containing_Nanopelagicus_viral_gn
## Store the highest y3 value mean
highest_y3_values = []
with open(f"{Path(output_directory) / 'no_katG_containing_Nanopelagicus_viral_gn.year2highest_y_value.txt'}", 'r') as lines:
    for line in lines:
        line = line.rstrip('\n')
...
## Store interpolation results
interpolation_result_files3 = glob(f"{Path(output_directory) / '*.no_katG_containing_Nanopelagicus_viral_gn.interpolation_results.txt'}")
x_data3 = x_mean
...
## Plot the second and third mean line (katG_containing_Nanopelagicus_viral_gn + no_katG_containing_Nanopelagicus_viral_gn)
...
plt.savefig('Nanopelagicus_viral_gn.combined.mean_lines.pdf')
...
## Save the combined figure
pltt.savefig('Nanopelagicus_MAG.mean_line.v2.pdf')