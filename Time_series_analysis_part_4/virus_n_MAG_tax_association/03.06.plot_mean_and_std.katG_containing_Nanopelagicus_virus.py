...
# Step 1 Store the katG-containing Nanopelagicus viral gn cov files for each year
katG_containing_Nanopelagicus_viral_gn_cov_files = glob("/storage1/data11/TYMEFLIES_phage/virus_n_MAG_tax_association/Nanopelagicus_virus_n_MAG/*.katG_containing_Nanopelagicus_viral_gn_cov.txt")
year2katG_containing_Nanopelagicus_viral_gn_cov = defaultdict(dict)
for katG_containing_Nanopelagicus_viral_gn_cov_file in katG_containing_Nanopelagicus_viral_gn_cov_files:
    year = Path(katG_containing_Nanopelagicus_viral_gn_cov_file).stem.split('.')[0]    
    x_list = []
    y_list = []
    with open(katG_containing_Nanopelagicus_viral_gn_cov_file, 'r') as lines:
        for line in lines:
            line = line.rstrip('\n')
            if not line.startswith('clearwater'):
                tmp = line.split('\t')
                x_list.append(float(tmp[0]))  # Convert x value to float
                y_list.append(float(tmp[1]))  # Convert y value to float
    year2katG_containing_Nanopelagicus_viral_gn_cov[year]['x'] = x_list 
    year2katG_containing_Nanopelagicus_viral_gn_cov[year]['y'] = y_list       


# Step 2 Generate the mean line
## Create interpolation functions for each year's dataset
interpolation_functions = {}
for year, data in year2katG_containing_Nanopelagicus_viral_gn_cov.items():
...
output_directory = "Nanopelagicus_virus_n_MAG"  # Replace with your desired output directory

year2highest_y_value = {} # year => highest_y_value
for year, data in interpolation_results.items():
    output_filename = f"{year}.katG_containing_Nanopelagicus_viral_gn.interpolation_results.txt"
...
f = open('Nanopelagicus_virus_n_MAG/katG_containing_Nanopelagicus_viral_gn.year2highest_y_value.txt', 'w')
...
interpolation_result_files = glob(f"{Path(output_directory) / '*.katG_containing_Nanopelagicus_viral_gn.interpolation_results.txt'}")
x_data = x_mean
...
plt.legend()
plt.grid(True)          
plt.savefig('katG_containing_Nanopelagicus_viral_gn.mean_line.pdf')


# Step 7 Get the Early Summer, Late Summer, Fall, and Ice-on dates bar plots
## Step 7.1 Store each year's Early Summer, Late Summer, Fall, and Ice-on start datetime
...
with open('/storage1/data11/TYMEFLIES_phage/season_start_dates.txt', 'r') as lines:
    for line in lines:
        if not line.startswith('Year'):
            ...
            earlysummer_start_day, latesummer_start_day, fall_start_day, iceon_start_day = line.split('\t')[3], line.split('\t')[4], line.split('\t')[5], line.split('\t')[6]
...
## Step 7.2 Calculate the Early Summer, Late Summer, Fall, and Ice-on bars
earlysummer_distance_list = []
latesummer_distance_list = []
fall_distance_list = []
iceon_distance_list = []
for year in year2clearwater_start_day:          
    ...
    date2 = datetime.strptime(earlysummer_start_day, "%Y-%m-%d")
    date3 = datetime.strptime(latesummer_start_day, "%Y-%m-%d")
    date4 = datetime.strptime(fall_start_day, "%Y-%m-%d")
    date5 = datetime.strptime(iceon_start_day, "%Y-%m-%d")
    earlysummer_distance_between_dates = date2 - date1
    latesummer_distance_between_dates = date3 - date1
    fall_distance_between_dates = date4 - date1
    iceon_distance_between_dates = date5 - date1
...
### Save the plot
plt.savefig('Nanopelagicus_virus_n_MAG.earlysummer_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Nanopelagicus_virus_n_MAG.latesummer_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Nanopelagicus_virus_n_MAG.fall_distance_barplot.pdf')
...
### Save the plot
plt.savefig('Nanopelagicus_virus_n_MAG.iceon_distance_barplot.pdf')
