...
# Aim: Parse to get katG virus abundance from 0 day of Clearwater
...
# Step 2 Get katG-containing viral gn list (species representatives)
Species = {}  # $gn_rep => $gns
AMG_summary = {}  # $pro => $ko
KOs = {}  # $ko => 1
...
## Step 2.3 Get katG-containing viral genome
for pro in sorted(AMG_summary.keys()):
    gn = pro.split("__Ga")[0]
    ko = AMG_summary[pro]
...
# Step 4 Calculate IMG2katG_viral_gn_cov dict
IMG2katG_viral_gn_cov = {} # IMG => katG_viral_gn_cov (The sum cov of katG-containing viral gn (rep))            
for IMG in IMG2date:
    katG_viral_gn_cov = 0
    for viral_gn in viral_gn2IMG2cov:
        if viral_gn in KatG_containing_viral_gn:
            cov = viral_gn2IMG2cov[viral_gn][IMG]
            katG_viral_gn_cov += float(cov)
    IMG2katG_viral_gn_cov[IMG] = katG_viral_gn_cov    
         
        
# Step 5 Calculate clearwater_day2katG_viral_gn_cov dicts for each year
os.mkdir("virus_n_MAG_tax_association/katG_virus_n_MAG")
for year in year2IMGs:
    IMGs = year2IMGs[year]
    clearwater_day2katG_viral_gn_cov = {} # int(clearwater_day) => katG_viral_gn_cov
    for IMG in IMGs:
        clearwater_day = int(IMG2clearwater_day[IMG])
        katG_viral_gn_cov = IMG2katG_viral_gn_cov[IMG]
        clearwater_day2katG_viral_gn_cov[clearwater_day] = katG_viral_gn_cov

    ## Write down the result
    f = open(f"virus_n_MAG_tax_association/katG_virus_n_MAG/{year}.katG_viral_gn_cov.txt", 'w')
    f.write('clearwater_day\tkatG_viral_gn_cov\n')
    for clearwater_day in sorted(clearwater_day2katG_viral_gn_cov.keys()):
        line = str(clearwater_day) + '\t' + str(clearwater_day2katG_viral_gn_cov[clearwater_day])
        f.write(line + '\n')