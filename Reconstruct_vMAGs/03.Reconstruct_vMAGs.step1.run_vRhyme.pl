...
foreach my $img_id (sort keys %IMGID){
	...
	if (!(-d "$img_id/vRhyme_result") and -d "$img_id\/VIBRANT\_$img_id\.a\/VIBRANT\_results\_$img_id.a"){
		...
		#print OUTT "gzip -dc $read_dir/$img_id.filtered.fastq.gz | bash deinterleave_fastq.sh $read_dir/$img_id.filtered_1.fastq $read_dir/$img_id.filtered_2.fastq;";
				
		# run vRhyme
		my $threads = 1;
		print OUTT "vRhyme -i $img_id/VIBRANT_$img_id.a.v2.min2000/$img_id.a.phage_scaffold.fna -t $threads -r $read_dir/$img_id.filtered*.fastq -o $img_id/vRhyme_result\n";
		#print OUTT "rm $read_dir/$img_id.filtered_1.fastq $read_dir/$img_id.filtered_2.fastq\n";
	}
}
close OUTT;
