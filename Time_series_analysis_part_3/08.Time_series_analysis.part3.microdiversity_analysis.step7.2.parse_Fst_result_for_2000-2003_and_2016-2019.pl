...
# Aim: Parse Fst result for year 2000-2003 (at the 1st place) and 2016-2019 (at the 2nd place)

# Step 1 Store Fst result of each gene
my %Fst = (); # $gene => [0] $fst [1] $pi_1 [2] $pi_2 [3] $cov_1 [4] $cov_2
open IN, "ls viral_species_rep_bams.for_each_year/Fst_for_each_viral_gn/*.tsv |";
while (<IN>){
	chomp;
	my $file = $_;
	open INN, "$file";
...
open IN, "viral_species_rep_bams.for_each_year/2000-2003.IS/output/2000-2003.IS_gene_info.tsv";
while (<IN>){
	chomp;
	if (!/^scaffold/){
		my @tmp = split (/\t/);
...
open IN, "viral_species_rep_bams.for_each_year/2016-2019.IS/output/2016-2019.IS_gene_info.tsv";
while (<IN>){
	chomp;
	if (!/^scaffold/){
		my @tmp = split (/\t/);
...
open OUT, ">viral_species_rep_bams.for_each_year/Fst_for_each_viral_gn/All_filtered_Fst_result.txt";
