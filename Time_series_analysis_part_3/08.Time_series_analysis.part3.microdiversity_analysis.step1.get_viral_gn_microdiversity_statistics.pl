...
# Step 6 Get Viral gn 2 month 2 nucleotide diversity result and 2 year_month 2 nucleotide diversity result
## Step 6.1 Make %Viral_gn2year_month2nt_diversity
my %Viral_gn2year_month2nt_diversity = (); # $viral_gn => $year_month => $nt_diversity
foreach my $viral_gn (sort keys %Viral_gn2distribution_n_cov_mean){
	...
	if ($distribution >= 20){
		foreach my $year_month (sort keys %Year_month2img_id){
			...
			my $nt_diversity_for_this_year_month = "NA"; # Store the nt diversity for this year_month (the mean value of all non-"NA" values)
			...
			if (@Nt_diversity_collection){
				$nt_diversity_for_this_year_month = mean(@Nt_diversity_collection);
			}
			$Viral_gn2year_month2nt_diversity{$viral_gn}{$year_month} = $nt_diversity_for_this_year_month;
		}
	}
}

## Step 6.2 Write down %Viral_gn2year_month2nt_diversity
open OUT, ">MetaPop/Viral_gn2year_month2nt_diversity.txt";
my $row2=join("\t", @Year_month);
print OUT "Head\t$row2\n";
foreach my $tmp1 (sort keys %Viral_gn2year_month2nt_diversity){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (@Year_month) {       
                if (exists $Viral_gn2year_month2nt_diversity{$tmp1}{$tmp2}){
                        push @tmp, $Viral_gn2year_month2nt_diversity{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"NA";
                }
        }
...
## Step 6.3 Write down Viral_gn2year_month2nt_diversity.four_AMGs.txt
open OUT, ">MetaPop/Viral_gn2year_month2nt_diversity.four_AMGs.txt";
...
## Step 6.4 Make %Viral_gn2month2nt_diversity
my %Viral_gn2month2nt_diversity = (); # $viral_gn => $month => $nt_diversity
foreach my $viral_gn (sort keys %Viral_gn2distribution_n_cov_mean){
	my $distribution = $Viral_gn2distribution_n_cov_mean{$viral_gn}[0];
	if ($distribution >= 20){
		foreach my $month (sort keys %Month2img_id){
			...
			my @Nt_diversity_collection = (); # Store all the non-"NA" nt diversity values 
			my $nt_diversity_for_this_month = "NA"; # Store the nt diversity for this month (the mean value of all non-"NA" values)
			foreach my $img (@IMG_ID){
				my $nt_diversity = "NA";
				if (exists $Viral_gn2IMG2nt_diversity{$viral_gn}{$img}){
					$nt_diversity = $Viral_gn2IMG2nt_diversity{$viral_gn}{$img};
			...
			if (@Nt_diversity_collection){
				$nt_diversity_for_this_month = mean(@Nt_diversity_collection);
			}
			$Viral_gn2month2nt_diversity{$viral_gn}{$month} = $nt_diversity_for_this_month;
		}
	}
}

## Step 6.5 Write down %Viral_gn2month2nt_diversity
open OUT, ">MetaPop/Viral_gn2month2nt_diversity.txt";
my $row3=join("\t", sort keys %Month2num);
print OUT "Head\t$row3\n";
foreach my $tmp1 (sort keys %Viral_gn2month2nt_diversity){
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (sort keys %Month2num) {       
                if (exists $Viral_gn2month2nt_diversity{$tmp1}{$tmp2}){
                        push @tmp, $Viral_gn2month2nt_diversity{$tmp1}{$tmp2};
                }else{              
                        push @tmp,"NA";
                }
        }
...
## Step 6.6 Write down Viral_gn2month2nt_diversity.four_AMGs.txt
open OUT, ">MetaPop/Viral_gn2month2nt_diversity.four_AMGs.txt";
